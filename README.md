...
Python 3.9 이상. 핵심 의존성은 `trimesh`, `shapely`, `numpy`, `networkx` 이고,
...
| 옵션 | 기본값 | 설명 |
|------|--------|------|
| `--nozzle` | 1.0 | 서포터 익스트루더 노즐 지름 = 구 지름 기준 |
...
- 3MF 를 열려면 `networkx` 가 필요합니다. trimesh 의 선택 의존성이라 예전 버전을
  설치하셨다면 `pip install networkx` 를 한 번 실행하세요. (STL 은 없어도 열립니다.)
